...
@section('title')
    Salary Details - Employee Management App
@endsection
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6 fade-in">
            <nav class="flex items-center space-x-2 text-sm text-gray-600">
                ...
                <span class="text-gray-900 font-medium">Salary Details</span>
            </nav>
        </div>
        ...
        <div class="bg-white rounded-2xl card-shadow overflow-hidden fade-in mb-6">
                ...
                <div class="relative">
                    <div class="flex items-center space-x-4 mb-4">
                        <div class="bg-white bg-opacity-20 backdrop-blur-sm p-4 rounded-xl shadow-lg">
                            <span class="material-icons text-blue-500 text-3xl">receipt_long</span>
                        </div>
                        <div>
                            <p class="text-blue-100 text-sm font-medium mb-1">Salary Slip for {{ $salary->bulan }}</p>
                            <h1 class="text-3xl font-bold text-white">{{ $salary->employee->nama_lengkap ?? 'N/A' }}</h1>
                        </div>
                    </div>
                    <div class="text-blue-600 font-semibold text-2xl bg-white bg-opacity-20 backdrop-blur-sm px-4 py-2 rounded-lg inline-block">
                        Total: Rp {{ number_format($salary->total_gaji, 2, ',', '.') }}
                    </div>
                </div>
            </div>
            ...
            <div class="p-8">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div class="info-card bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200">
                        <p class="text-sm text-gray-600 font-medium mb-1">Base Salary</p>
                        <p class="text-3xl font-bold text-gray-900">Rp {{ number_format($salary->gaji_pokok, 2, ',', '.') }}</p>
                    </div>
                    <div class="info-card bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-6 border border-green-200">
                        <p class="text-sm text-green-600 font-medium mb-1">Allowance</p>
                        <p class="text-3xl font-bold text-green-900">+ Rp {{ number_format($salary->tunjangan, 2, ',', '.') }}</p>
                    </div>
                    <div class="info-card bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-6 border border-red-200">
                        <p class="text-sm text-red-600 font-medium mb-1">Deduction</p>
                        <p class="text-3xl font-bold text-red-900">- Rp {{ number_format($salary->potongan, 2, ',', '.') }}</p>
                    </div>
                </div>
