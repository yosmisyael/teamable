...
@section('title')
    Create Salary Slip - Employee Management App
@endsection
...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6 fade-in">
            <nav class="flex items-center space-x-2 text-sm text-gray-600">
                ...
                <span class="text-gray-900 font-medium">Create New Salary Slip</span>
            </nav>
        </div>
            ...
            <div class="bg-white rounded-2xl card-shadow p-8">
                <div class="flex items-center mb-8 pb-6 border-b border-gray-200">
                    <div class="bg-gradient-to-br from-blue-500 to-blue-600 py-3 px-4 rounded-full mr-4 shadow-lg">
                        <span class="material-icons text-white text-xl">add_card</span>
                    </div>
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">Create Salary Slip</h2>
                        <p class="text-sm text-gray-600 mt-1">Fill in the details to generate a new salary record</p>
                    </div>
                </div>

                <form action="{{ route('salaries.store') }}" method="POST" class="space-y-8">
                    @csrf
                    ...
                    <div class="input-container relative">
                        ...
                        <label for="bulan" class="floating-label absolute left-4 top-3 text-gray-500 pointer-events-none">
                            Salary Period (Month and Year)
                        </label>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="input-container relative">
                            <input type="number" id="gaji_pokok" name="gaji_pokok" step="0.01" min="0" required value="{{ old('gaji_pokok') }}"
                        ...
                        <div class="input-container relative">
                            <input type="number" id="tunjangan" name="tunjangan" step="0.01" min="0" required value="{{ old('tunjangan', 0) }}"
                                   class="salary-component material-input w-full px-4 pt-4 pb-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none peer"
                                   onfocus="toggleLabel(this, true)" onblur="toggleLabel(this, false)">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="input-container relative">
                            <input type="number" id="potongan" name="potongan" step="0.01" min="0" required value="{{ old('potongan', 0) }}"
                                   class="salary-component material-input w-full px-4 pt-4 pb-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none peer"
                                   onfocus="toggleLabel(this, true)" onblur="toggleLabel(this, false)">
    ...
    <script>
        ...
        function calculateTotalSalary() {
            ...
            toggleLabel(totalInput, false); // Update label state for total field
        }
