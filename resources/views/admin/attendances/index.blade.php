...
@section('content')
        ...
        <div class="bg-white rounded-2xl card-shadow p-6 mb-8 fade-in">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">Attendance Log</h2>
                        <p class="text-sm text-gray-600 mt-1">Track and manage employee attendance</p>
                    </div>
                </div>
                    ...
                    @forelse($attendances as $attendance)
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {{-- Assumes you eager load the employee: Attendance::with('employee')->get() --}}
                                {{ $attendance->employee->nama_lengkap ?? 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ \Carbon\Carbon::parse($attendance->tanggal)->format('d F Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $attendance->waktu_masuk ? \Carbon\Carbon::parse($attendance->waktu_masuk)->format('H:i') : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $attendance->waktu_keluar ? \Carbon\Carbon::parse($attendance->waktu_keluar)->format('H:i') : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusClasses[$attendance->status_absensi] ?? 'bg-gray-100 text-gray-800' }}">
                                        {{ ucfirst($attendance->status_absensi) }}
                                    </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                <a href="{{ route('attendances.show', $attendance->id) }}" class="p-2 bg-blue-50 text-blue-600 rounded-md hover:bg-blue-100 transition-colors" title="View Detail">
                                    <span class="material-icons text-sm">visibility</span>
                                </a>
                                <a href="{{ route('attendances.edit', $attendance->id) }}" class="p-2 bg-amber-50 text-amber-600 rounded-md hover:bg-amber-100 transition-colors" title="Edit Record">
                                    <span class="material-icons text-sm">edit</span>
                                </a>
                                <button onclick="openDeleteModal({{ $attendance->id }}, '{{ addslashes($attendance->employee->nama_lengkap ?? 'N/A') }} on {{ $attendance->tanggal }}')" class="p-2 bg-red-50 text-red-600 rounded-md hover:bg-red-100 transition-colors" title="Delete Record">
                                    <span class="material-icons text-sm">delete</span>
                                </button>
                                <form id="deleteAttendanceForm{{ $attendance->id }}" action="{{ route('attendances.destroy', $attendance->id) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500">
                                <span class="material-icons text-4xl text-gray-300">event_note</span>
                                <p class="mt-2 font-medium">No attendance records found</p>
                                <p class="mt-1">Get started by adding a new attendance record.</p>
                            </td>
                        </tr>
                    @endforelse
                    </tbody>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-2xl p-8 max-w-md w-full mx-4 card-shadow">
            ...
            <p class="text-gray-700 mb-8">
                Are you sure you want to delete the record for <span id="attendanceIdentifier" class="font-semibold"></span>?
            </p>
    ...
    <script>
        ...
        const attendanceIdentifierSpan = document.getElementById('attendanceIdentifier');

        function openDeleteModal(attendanceId, identifier) {
            formToSubmitId = `deleteAttendanceForm${attendanceId}`;
            attendanceIdentifierSpan.textContent = identifier;
